...
class IcebergCalving:
    ...
    def _get_data(self):
        ...
        ######## LU
        tmp = self.data * self.res * self.res  # kg/m2year * m *m
        total = tmp.sum() /-1e12   # kg/m2year * m *m /1e12-> Gt/year
        print("******** Info: Total discharge amount in Gt/year:", total)
    ...
    ######################################
        # loop over all bins
        with tqdm(total=len(self.scaling_factor), file=sys.stdout, desc='go through all bins') as pbarbin:
            ...
            for index in self.df_agg.index:
                ...
                while True:
                    ...
                    if isinstance(ib_tmp, int):
                        ...
                        print("[INFO] ib_tmp is empty. Skipping basin.") # Lu
                        break                   # now equivalent to continue
                    else:
                        print("[INFO] Successfully generated iceberg data.") # Lu
                    ...
                    if len(felems) != 0:
                        ...
                        with tqdm(total=len(self.df_agg), file=sys.stdout, desc='initialize icebergs') as pbar2:
